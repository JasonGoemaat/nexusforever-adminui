<script lang="ts">
    ...
    import { addLocalizedText } from "$lib/Text";

    const { id } = $props();
    let objective = $state<any>(null);

    onMount(async () => {
        console.log(`QuestObjective(${id}): onMount()`)
        const objectiveDetail = await getJson(`tables/QuestObjective/${id}`)
        await addLocalizedText(objectiveDetail)
...
<li>Objective {id}</li>
{:else}
<li>{id} - {objective.Short} - {objective.Full}</li>
<ul>
    {#if objective.Type === 2}
    <li>Kill {objective.Count} Creature {objective.Data} </li>
    {:else if objective.Type === 3}
    <li>Talk To Creature {objective.Data}</li>
    {:else if objective.Type === 4}
    <li>Collect {objective.Count} Item {objective.Data}</li>
    {:else if objective.Type === 2}
    <li>SuccesedCSI {objective.Data} ({objective.Count}) - CreatureId, ObjectiveText describes interacting with object after ClientSideInteraction event - i.e. planting seeds in fertile soil</li>
    {:else if objective.Type === 24}
    <li>Quest {objective.Data}</li>
    {:else if objective.Type === 32}
    <li>VirtualCollect {objective.Count} Item {objective.Data}</li>
    {:else}
    <li>Unknown objective type {objective.Type} - data is {objective.Data}</li>
    {/if}
</ul>
{/if}
